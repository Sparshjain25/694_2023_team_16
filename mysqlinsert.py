...
with open("corona_out_3", "r") as f1:
    for line in f1:
        try:
            data = json.loads(line)
            if (data['text'].startswith('RT')):
                #adding retweet user in database
                ...
                #adding quoted user in database
                ...
                #adding retweeted user in database
                ...
                #adding tweeted user in database
                cursor.execute("SELECT * FROM user WHERE id = %s", (data['user']['id_str'],))
                if (cursor.fetchone() is None):
                    user = data['user']
                    user_id = user['id_str']
